...
function nouisliderInit(filterSliderWrapperSelector, filterSliderSelector, leftValueSelector, rightValueSelector) {
  document.querySelectorAll(filterSliderWrapperSelector).forEach(function (filterSliderWrapper) {
    var filterSlider = filterSliderWrapper.querySelector(filterSliderSelector);
    var minValue = Number(filterSlider.getAttribute('data-min'));
    var maxValue = Number(filterSlider.getAttribute('data-max'));
    var handles = filterSlider.getAttribute('data-handles').split(',').map(function (handle) {
      return Number(handle.trim());
    });
    var unit = filterSlider.getAttribute('data-unit');
    var tooltips = [];
    if (handles.length >= 2) {
      handles.splice(2);
      tooltips = [false, {
        to: function to(value) {
          return Math.floor(value) + " ".concat(unit);
        }
      }];
    } else {
      var leftValueWrapper = filterSliderWrapper.querySelector(leftValueSelector).parentElement;
    ...
    var leftValue = filterSliderWrapper.querySelector(leftValueSelector);
    var rightValue = filterSliderWrapper.querySelector(rightValueSelector);
...
function bindListenerToFilterCategories(filterCategorySelector, filterCategoryInputSelector) {
  document.querySelectorAll(filterCategorySelector).forEach(function (categoryFilter) {
    categoryFilter.addEventListener('click', function (e) {
      if (categoryFilter.classList.contains("".concat(filterCategorySelector.slice(1), "--checked"))) {
        categoryFilter.classList.remove("".concat(filterCategorySelector.slice(1), "--checked"));
        categoryFilter.querySelector(filterCategoryInputSelector).checked = false;
      } else {
        categoryFilter.classList.add("".concat(filterCategorySelector.slice(1), "--checked"));
        categoryFilter.querySelector(filterCategoryInputSelector).checked = true;
      }
    });
  });
}
function bindListenerToFilters(productFiltersSelector, productLabelSelector, productInputSelector, choosenFiltersSelector, choosenFilterSelector) {
  document.querySelectorAll(productLabelSelector).forEach(function (productFilterLabel) {
    productFilterLabel.addEventListener('click', function (e) {
      var productFilterValue = productFilterLabel.previousElementSibling.value;
      document.querySelectorAll(choosenFiltersSelector).forEach(function (choosenFilters) {
        var choosenFilter = choosenFilters.querySelector("".concat(choosenFilterSelector, "[data-value=\"").concat(productFilterValue, "\"]"));
        if (!choosenFilter) {
          choosenFilters.insertAdjacentHTML('beforeend', getChoosenProductFilterElement(productFilterValue));
          bindListenerToChoosenFilter(choosenFilters.querySelector("".concat(choosenFilterSelector, "[data-value=\"").concat(productFilterValue, "\"]")), productFiltersSelector, productInputSelector);
        } else {
...
function bindListenerToChoosenFilter(choosenFilter, productFiltersSelector, productInputSelector) {
  choosenFilter.addEventListener('click', function (e) {
    var productFilterValue = choosenFilter.getAttribute('data-value');
    document.querySelectorAll(productFiltersSelector).forEach(function (productFilter) {
      var productFilterInput = productFilter.querySelector("".concat(productInputSelector, "[value=\"").concat(productFilterValue, "\"]"));
...
function getChoosenProductFilterElement(value) {
  return "<div class=\"choosen-filters__filter\" data-value=\"".concat(value, "\">\n            <div class=\"choosen-filters__name\">").concat(value, "</div>\n            <svg class=\"choosen-filters__remove-icon\">\n              <use xlink:href=\"#cancel\"></use>\n            </svg>\n          </div>");
}
function bindListenerToFiltersAccordeon() {
  document.querySelectorAll('.product-filters').forEach(function (productFilter) {
    productFilter.querySelectorAll('.product-filters__section-header').forEach(function (header) {
...
function showProductFilters(section) {
  var filters = section.querySelector('.product-filters__filters');
  var tabIcon = section.querySelector('.product-filters__tab-icon');
  ...
  filters.style.height = "".concat(height, "px");
...
function hideProductFilters(section) {
  ...
  filters.style.height = "".concat(filters.offsetHeight, "px");
  filters.offsetHeight;
  filters.style.display = 'block';
  filters.style.height = 0;
  filters.style.overflow = 'hidden';
...
function bindListenerToSort(buttonSelector, tabSelector) {
  ...
  button.addEventListener('click', function (e) {
    if (tab.classList.contains("".concat(tabSelector.slice(1), "--show"))) {
      tab.classList.remove("".concat(tabSelector.slice(1), "--show"));
    } else {
      tab.classList.add("".concat(tabSelector.slice(1), "--show"));
    }
  });
}
window.addEventListener('load', function () {
  embedSprite('assets/icons/clean/sprite.svg', '.layout');
  nouisliderInit('.filter-sliders__slider-wrapper', '.filter-sliders__slider', '.filter-sliders__value-wrapper--left .filter-sliders__value', '.filter-sliders__value-wrapper--right .filter-sliders__value');
  bindListenerToFilterCategories('.filter-categories__category', '.filter-categories__input');
  bindListenerToFilters('.product-filters', '.product-filters__filter-label', '.product-filters__filter-input', '.choosen-filters', '.choosen-filters__filter');
